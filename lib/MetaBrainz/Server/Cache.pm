...
#
#   MetaBrainz -- the open internet music database
#
...
package MetaBrainz::Server::Cache;
...
{
	my ($class, $key, @args) = @_;
	my $timer = MetaBrainz::Server::CacheTimer->new("get", $key, @args) if CACHE_TIMER;
...
{
	my ($class, $key, @args) = @_;
	my $timer = MetaBrainz::Server::CacheTimer->new("set", $key, @args) if CACHE_TIMER;
...
{
	my ($class, $key, @args) = @_;
	my $timer = MetaBrainz::Server::CacheTimer->new("add", $key, @args) if CACHE_TIMER;
...
{
	my ($class, $key, @args) = @_;
	my $timer = MetaBrainz::Server::CacheTimer->new("replace", $key, @args) if CACHE_TIMER;
...
This module implements a Cache::Cache layer for MetaBrainz.  The layer implementing
the cache (e.g. File, MemCached, etc) is not known to the caller.  This layer
supports get / set / add / replace / delete.

=cut

package MetaBrainz::Server::CacheTimer;
