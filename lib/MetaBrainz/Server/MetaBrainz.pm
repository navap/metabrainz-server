...
package MetaBrainz::Server::MetaBrainz;
...
use MetaBrainz::Server::Database;
...
sub Login
{
	...
	my $db = $opts{'db'};
	$db = "READWRITE" if not defined $db;
	unless (ref $db)
	{
		$db = MetaBrainz::Server::Database->get($db)
   ...
   return 0 if (!$this->{DBH});
	...
	{
		require MetaBrainz::Server::Sql;
		my $sql = MetaBrainz::Server::Sql->new($this->{DBH});
